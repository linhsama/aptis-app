...
function renderQuestion4(index) {
    ...
    // Giao diện Ver 2: Gộp câu hỏi vào dưới từng đoạn văn tương ứng
    const leftColumn = row.querySelector('.col-md-7') || row.querySelector('.col-md-12');
    if (leftColumn) {
        leftColumn.className = 'col-md-12'; // Mở rộng full width
        leftColumn.innerHTML = ''; // Xóa nội dung cũ
        
        // Render hướng dẫn (phần tử đầu tiên)
        const instruction = document.createElement('p');
        instruction.innerHTML = question4Text[index][0];
        leftColumn.appendChild(instruction);
        
        // Các chữ cái tương ứng với đoạn văn
        const paragraphLetters = ['A', 'B', 'C', 'D'];
        
        // Lặp qua 4 đoạn văn (từ index 1 đến 4 trong mảng text)
        for (let i = 1; i <= 4; i++) {
            if (!question4Text[index][i]) continue;
            
            const letter = paragraphLetters[i - 1];
            const paraDiv = document.createElement('div');
            paraDiv.classList.add('mb-4', 'p-3', 'border', 'rounded', 'bg-light');
            
            // Nội dung đoạn văn
            const paraText = document.createElement('p');
            paraText.innerHTML = question4Text[index][i];
            paraDiv.appendChild(paraText);
            
            // Tìm các câu hỏi thuộc đoạn văn này
            const matchingQuestions = [];
            const correctAnswers = correctAnswersQuestion4[index];
            const questions = question4Content[index];
            
            for (let j = 0; j < questions.length; j++) {
                if (correctAnswers[j] === letter) {
                    matchingQuestions.push(questions[j].question);
                }
            }
            
            // Hiển thị danh sách câu hỏi liên quan
            if (matchingQuestions.length > 0) {
                const qsTitle = document.createElement('h6');
                qsTitle.classList.add('text-primary', 'mt-3');
                qsTitle.innerHTML = '<strong>Các câu hỏi thuộc đoạn này:</strong>';
                paraDiv.appendChild(qsTitle);
                ...
                paraDiv.appendChild(ul);
            }
            
            leftColumn.appendChild(paraDiv);
        }
    }

    // Ẩn cột bên phải (form câu hỏi) vì không còn sử dụng
    const rightColumn = row.querySelector('.col-md-5') || row.querySelector('.d-none');
    if (rightColumn) {
        rightColumn.className = 'd-none';
        const form = rightColumn.querySelector('form');
        if (form) form.innerHTML = ''; // Xóa nội dung cũ
    }
