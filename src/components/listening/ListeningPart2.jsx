...
const getOriginalQuestionNumber = (question) => {
  ...
  const origIndex = part2Data.findIndex(q => q.id === question.id);
...
const ListeningPart2 = () => {
  ...
  const { weakIds, slowIds, masteredIds, prefilledStatuses } = useMemo(() => {
    ...
    const sortedHistory = [...history]
      .filter(h => (!h.skill || h.skill === 'listening') && (h.part === 'part-2'))
  ...
  const allValidQuestions = useMemo(() => {
    return part2Data.filter(q => q.speakers && q.speakers.length > 0);
  }, []);
  ...
  useEffect(() => {
    if (isStudy && currentQuestion && !allChecked[currentQuestion.id]) {
      const autoAns = {};
      currentQuestion.speakers.forEach(s => {
        autoAns[s.id] = s.answer;
      });
  ...
  const handleSelectSpeakerOption = (speakerId, optionValue) => {
  ...
  const isAllSelected = currentQuestion?.speakers?.every(s => currentAnswers[s.id]);
  ...
  const handleCheck = useCallback(() => {
    ...
    currentQuestion.speakers.forEach(s => {
      ...
      if (userChoice === s.answer) {
        currentScore++;
      }
    });

    const timeSpent = Math.max(1, Math.floor((Date.now() - startTime) / 1000));
    const isPerfect = currentScore === currentQuestion.speakers.length;
  ...
  const handleResetAll = () => {
    ...
    try {
      ...
      if (historyString) {
        ...
        const part2Ids = new Set(allValidQuestions.map(q => q.id));
        const updated = history.filter(h => h.part !== 'part-2' && !part2Ids.has(h.id));
        localStorage.setItem('aptis_listening_history', JSON.stringify(updated));
      }
    } catch (e) {
      console.error('Failed to clear part 2 listening history', e);
    }
    ...
    return (
      <div className="practice-container">
        <div className="panel-card" style={{ padding: '3rem', textAlign: 'center' }}>
          ...
          <button onClick={() => navigate('/practice/listening/part-2')} className="btn btn-primary" style={{ margin: '1.5rem auto 0' }}>
  ...
  return (
    <div className="practice-container">
      ...
      <div className="practice-top-card">
        <div className="practice-header-row">
          <div className="practice-title-group">
            <span className="practice-badge-skill">Listening · Part 2</span>
            <div className="practice-topic-badge" title="Chủ đề">
              <Users size={14} color="var(--primary)" />
              <span>Đề {getOriginalQuestionNumber(currentQuestion)} / {allValidQuestions.length}: {currentQuestion.topic || 'Information Matching'}</span>
            </div>
            <Timer
              key={`exam-${resetCount}`}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', flexWrap: 'wrap' }}>
            ...
            <button
              ...
              title="Đặt lại toàn bộ bài tập Part 2"
              style={{ padding: '0.35rem 0.65rem' }}
            >
              <RotateCcw size={13} />
              <span>Reset</span>
      ...
      <div className="listening-grid-2col">
        {/* Left Card: Audio & Options Reference & Transcript */}
        <div className="listening-card-left">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border)', paddingBottom: '0.5rem' }}>
            <span style={{ fontSize: '0.82rem', fontWeight: 700, color: 'var(--primary)', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
              <Headphones size={15} />
              <span>Audio & Danh sách ý kiến</span>
            </span>
          ...
          {/* Reference Options List (A - F) */}
          <div style={{ background: '#f8fafc', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', padding: '0.75rem 1rem' }}>
            <div style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: '0.45rem' }}>
              Danh sách 6 ý kiến cần nối:
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
              {shuffledOptions.map((opt, oIdx) => (
                <div key={oIdx} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem', fontSize: '0.86rem', color: 'var(--text-main)' }}>
                  <span style={{
                    width: '20px',
                    height: '20px',
                    borderRadius: '4px',
        ...
        <div className="listening-card-right">
          ...
          <p className="text-muted" style={{ fontSize: '0.84rem', margin: 0 }}>
            Chọn ý kiến phát biểu phù hợp cho từng người nói (Person A - D):
          </p>

          {/* 4 Speakers List */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
            {currentQuestion.speakers.map((speaker) => {
              const userChoice = currentAnswers[speaker.id] || '';
              const isCorrect = isChecked ? userChoice === speaker.answer : null;

              let itemClass = 'speaker-matching-item';
              if (isChecked) {
              ...
              return (
                <div key={speaker.id} className={itemClass}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem', width: '100%' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <div className="speaker-label-badge">
                        <Users size={14} />
                        <span>{speaker.name}</span>
                      </div>

                      {isChecked && (
                        <span style={{ fontSize: '0.78rem', fontWeight: 700, color: isCorrect ? 'var(--success)' : 'var(--danger)', display: 'flex', alignItems: 'center', gap: '3px' }}>
                    ...
                    <select
                      ...
                      onChange={(e) => handleSelectSpeakerOption(speaker.id, e.target.value)}
                      style={{
                        width: '100%',
                        ...
                        background: '#ffffff',
                        color: userChoice ? 'var(--text-main)' : 'var(--text-muted)',
                        fontSize: '0.88rem',
                        outline: 'none',
                        cursor: isChecked ? 'default' : 'pointer'
                      }}
                    >
                      <option value="">-- Chọn phát biểu của {speaker.name} --</option>
                      {shuffledOptions.map((opt, oIdx) => (
                    ...
                    {isChecked && !isCorrect && (
                      <div style={{ fontSize: '0.8rem', color: 'var(--success)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '3px', marginTop: '2px' }}>
                        <Check size={12} /> Đáp án đúng: {speaker.answer}
                      </div>
                    )}
                  </div>
                </div>
        ...
        <div className="drawer-overlay" onClick={() => setShowResultPopup(false)}>
          <div className="result-drawer" onClick={e => e.stopPropagation()}>
            <div className="drawer-header">
              <h2 style={{ fontSize: '1.25rem', fontWeight: 700, margin: 0 }}>Kết quả luyện tập Part 2</h2>
